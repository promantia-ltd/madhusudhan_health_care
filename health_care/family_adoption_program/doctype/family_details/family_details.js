frappe.ui.form.on("Family Details", {
	...
	refresh(frm) {
		...
		const migration_reason_field = frm.fields_dict['migration_reason'].input;
        if (migration_reason_field) {
            $(migration_reason_field).on('focus', function () {
				console.log("testing onfocus")
				startVoiceRecording(frm,"migration_reason");
            });
			$(migration_reason_field).on('blur', function () {
				console.log("testing on blur")
				stopVoiceRecording(frm);
            });
        }
		const any_remarks_field = frm.fields_dict['any_remarks'].input;
		if (any_remarks_field) {
            $(any_remarks_field).on('focus', function () {
				startVoiceRecording(frm,"any_remarks");
            });
			$(any_remarks_field).on('blur', function () {
				stopVoiceRecording(frm);
            });
        }
